...
export default (editor, config = {}) => {
    ...
    let model = defaultModel.extend({
        ...
        defaults: Object.assign({}, defaultModel.prototype.defaults, {
            ...
            script: function () {
                ...
                document.dispatchEvent(new Event('llikeload'));
                ...
                child && child.addEventListener('click', function () {
                    window.open(`http://www.linkedin.com/shareArticle?mini=true&url=${ encodeURIComponent(url) }&text=${text}`, 'Linkedin Shared', 'left=0,top=0,width=420,height=620,personalbar=0,toolbar=0,scrollbars=0,resizable=0');
                });
            }
        }),
        ...
        isComponent: function (el) {
            var result = '';
            if (el.tagName === 'DIV' && el.className === `${config.prefix}-linkedin`) {
                result = {type: 'tlinkedin'};
            }
            return result;
        }
    });
    ...
    let view = defaultView.extend({
        ...
        init: function () {
            ...
            if (doc) {
                doc.addEventListener('llikeload', function () {
                    self.updatePosition();
                });
            }
